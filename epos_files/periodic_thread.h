...
class Periodic_Thread_SRP: public Periodic_Thread
{
public:
    template<typename ... Tn>
    Periodic_Thread_SRP(const Configuration & conf,int pi, int (* entry)(Tn ...), Tn ... an): Periodic_Thread(conf, entry), _preemption_level(pi)
    {
    }

    int preemption_level(){ return _preemption_level; }

private:
    int _preemption_level;
};
